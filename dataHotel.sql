...
CREATE TABLE IF NOT EXISTS `hotel`.`hotel` (
  ...
  `idHotelImage` INT(11) NULL DEFAULT NULL,
  `userName` VARCHAR(30) NULL DEFAULT NULL,
  `utilities` INT(11) NULL DEFAULT NULL,
  `rate` FLOAT NULL DEFAULT NULL,
  PRIMARY KEY (`idHotel`),
  INDEX `fk_hotel_account` (`userName` ASC) VISIBLE,
  INDEX `fk_hotel_image_idx` (`idHotelImage` ASC) VISIBLE,
  CONSTRAINT `fk_hotel_account`
    FOREIGN KEY (`userName`)
    REFERENCES `hotel`.`account` (`userName`),
  CONSTRAINT `fk_hotel_image`
    FOREIGN KEY (`idHotelImage`)
    REFERENCES `hotel`.`hotelimage` (`idHotelImage`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `hotel`.`bookingroom` (
  ...
  INDEX `fk_Maph_Phong_idx` (`idRoom` ASC) VISIBLE,
